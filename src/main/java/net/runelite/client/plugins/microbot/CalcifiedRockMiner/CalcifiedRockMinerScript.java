...
public class CalcifiedRockMinerScript extends Script {
    ...
    private static final int STAY_WITHIN_RADIUS = 6;
    ...
    private void handleMining(CalcifiedRockMinerConfig config) {
        ...
        if (config.maxPlayersInArea() > 0 && shouldHopWorld(config)) {
            Microbot.log("Too many players nearby, hopping world...");
        ...
        if (config.focusCrackedWaterDeposits() && !Rs2Player.isMoving()) {
            Rs2TileObjectModel weepingRock = Microbot.getRs2TileObjectCache().query()
                    .withId(WEEPING_ROCK)
                    .within(CALCIFIED_ROCK_LOCATION, STAY_WITHIN_RADIUS)
                    .first();
            if (weepingRock != null) {
                Rs2TileObjectModel tearRock = Microbot.getClientThread().invoke(() ->
                        Microbot.getRs2TileObjectCache().query()
                                ...
                                .first());
                if (tearRock != null) {
                    if (tearRock.getWorldLocation().distanceTo(CALCIFIED_ROCK_LOCATION) <= STAY_WITHIN_RADIUS) {
    ...
    private boolean shouldHopWorld(CalcifiedRockMinerConfig config) {
        try {
            ...
            long nearbyPlayers = Microbot.getClient().getTopLevelWorldView().players().stream()
                    .filter(p -> p != null)
                    .filter(p -> p != Microbot.getClient().getLocalPlayer())
                    .filter(p -> p.getWorldLocation().distanceTo(localLocation) <= 15)
                    .filter(p -> p.getAnimation() != -1)
                    .count();

            if (nearbyPlayers >= maxPlayers) {
                Microbot.log("Found " + nearbyPlayers + " other players nearby (max: " + maxPlayers + ")");
                return true;
            }
        } catch (Exception e) {
            return false;
    ...
    private void handleCrushing(CalcifiedRockMinerConfig config) {
        if (config.crushDeposits() && hasHammer() && Rs2Inventory.hasItem(29088)) {
            if (Rs2Player.getWorldLocation().distanceTo(ANVIL) <= 3) {
                ...
                Rs2Inventory.useItemOnObject(29088, anvil.getId());
                sleep(400, 600);
    ...
    private void handleBanking(CalcifiedRockMinerConfig config) {
        ...
        if (!Rs2Bank.isOpen()) {
            if (!Rs2Bank.isNearBank(BankLocation.CAM_TORUM, 10)) {
